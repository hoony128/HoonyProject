...
public class NoticePaging<T> implements Serializable{
	...
	public NoticePaging(int totalCount, int currentPage, int pageSize, int blockSize) {
	...
	public String  getPageList() {
		...
		// 이전 표시
		if(startPage>1) {
			message += "&nbsp;&nbsp;<span style=\"cursor:pointer; color:#338cff; font-size: 15\" onclick=\"post_to_url('notice', {'p':'" +(startPage-1)+"','s':'" + pageSize + "','b':'" + blockSize+"'});\">[이전]</span> ";
		...
		for(int i=startPage;i<=endPage;i++) {
			if(i==currentPage) {
				...
				message +="&nbsp;&nbsp;<span style=\"cursor:pointer; color:#338cff; font-size: 15\" onclick=\"post_to_url('notice', {'p':'" +i+"','s':'" + pageSize + "','b':'" + blockSize+"'});\">"+i+"</span> ";
			}
		}
		// 다음 표시
		if(endPage<totalPage) {
			message += "&nbsp;&nbsp;<span style=\"cursor:pointer; color:#338cff; font-size: 15\" onclick=\"post_to_url('notice', {'p':'" +(endPage+1)+"','s':'" + pageSize + "','b':'" + blockSize+"'});\">[다음]</span> ";
		}
		return message;
	}
	@Override
