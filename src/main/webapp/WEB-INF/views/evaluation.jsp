...
function a(formName){
	...
	f.action="evaluationInput";
	f.submit();
}
</script>

<title>${dname}부서 인사고과 명단</title>
</head>
<body>
<h1>${dname}부서</h1>		
<hr>		
			...
			<c:forEach var="vo" items="${dlist }" varStatus="vs"> 
				...
				<tr align="center">
					...
					<form name="formName${vs.count }" id="formName${vs.count }" method="post">
						...
						<c:if test="${vo.rw==1}">
						<input type="button" id="none" class="btn btn-danger" value="제출완료" onclick="a('formName${vs.count}');">
						</c:if>
						<c:if test="${vo.rw==0}">
						<input type="button" class="btn btn-primary" value="인사고과등록" onclick="a('formName${vs.count}');">
						</c:if>
